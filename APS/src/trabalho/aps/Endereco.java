...
public class Endereco {
	...
	public static int buscarDadosEndereco(int cdCliente) {
		String sql = "select nm_bairro, nm_cidade, nm_estado, de_rua, nr_residencial, de_complemento, nr_cep from tb_endereco INNER JOIN tb_cliente ON tb_endereco.cd_endereco = ('" + cdCliente + "');";
		Conexao con = new Conexao();
		...
		try {
			...
			consulta.next();
				...
				System.out.println("Bairro: " + nm_bairro);
				System.out.println("Cidade: " + nm_cidade);
				System.out.println("Estado: " + nm_estado);
				System.out.println("Rua: " + de_rua);
				System.out.println("Numero: " + nr_residencial);
				System.out.println("Complemento: " + de_complemento);
				System.out.println("Cep: " + nr_cep);
				return cdCliente;
		}catch(SQLException e) {
			e.printStackTrace();
		}
	...
	public static void updateEndereco(String cep, int numeroResidencial, String nomeBairro, String rua, String cidade, String estado,
		...
		int res = con.executeUpdate(sql);
			if (res >= 1) {
				...
				System.err.println("Atualização NÃO realizada!");
			}
	}
}
